...
function M.plugins_on_open()
  for name, opts in pairs(M.opts.plugins) do
    if name ~= "zen" and opts and opts.enabled then
...
function M.zen_on_open(winnr)
  local zen = plugins["zen"]
  M.state["zen"] = {}
  pcall(zen, M.state["zen"], true, M.zen_opts, winnr)
  M.zen = true
end

function M.zen_on_close()
  local zen = plugins["zen"]
  if not M.state["zen"] then
    return
  end
  pcall(zen, M.state["zen"], false, M.zen_opts)
  M.zen = false
end
...
function M.toggle_zen(opts)
  if M.is_open() then
    ...
    if M.zen then
      ...
      M.zen_on_open(vim.api.nvim_get_current_win())
    end
  end
end
...
function M.create(opts)
  ...
  M.plugins_on_open()
  if opts.args == "zen" then
    M.zen_on_open(M.parent)
  end
  ...
  for k, v in pairs(opts.window.options or {}) do
    vim.api.nvim_set_option_value(k, v, { M.win })
  end
  ...
  local augroup = [[
    augroup Focus
      autocmd!
      autocmd WinClosed %d ++once ++nested lua require("focus.view").close()
      autocmd WinEnter * lua require("focus.view").on_win_enter()
      autocmd CursorMoved * lua require("focus.view").fix_layout()
      autocmd VimResized * lua require("focus.view").fix_layout(true)
      autocmd CursorHold * lua require("focus.view").fix_layout()
      autocmd BufWinEnter * lua require("focus.view").on_buf_win_enter()
    augroup end]]
